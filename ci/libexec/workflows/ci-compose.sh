...
function ci-compose() {
  function createDocs() {
    cd "${PROJECT_ROOT}"
    dotnet tool restore
    local sourcePath="${BUILD_UNPACKAGED_DIST}/TestingUtils.Logging.dll"
    local outputPath="${BUILD_DOCS}/md"
    dotnet xmldocmd "${sourcePath}" "${outputPath}" \
      --namespace "Jds.TestingUtils.Logging" \
      --source "https://github.com/JeremiahSanders/testingutils-logging/tree/main/src" \
      --newline lf \
      --visibility public
  }
  ...
  printf "Composing build artifacts...\n\n" &&
    ...
    createDocs &&
    printf "\nMarkdown documentation output to %s\n\n" "${BUILD_DOCS}/md" &&
